...
\begin{minipage}[t]{0.48\textwidth}
   ...
   $p^{\mu ^{\prime}}= \left( m,0,0,0 \right)$, because the \\four-velocity in the rest frame is \\$u^{\mu^{\prime } }= \left( 1,0,0,0 \right)$.  \\
   What is the expression of $p^{\mu }$ in the (x,y,z) frame?  What is the fastest way to compute it?		
...
\]
\footnotetext{ $\delta \left( \vec{r} - \vec{r_{j}} \right)$ is the Dirac's delta function, here it is centered in $\vec{r_{j}}$. Its value is 0 everywhere but in $\vec{r}=\vec{r_{j}}$ where it assume value $\infty$. The function is normalized in such a way that the integral over the whole space(time?) region is 1. } 
...
If I change frame I have to change $\alpha , \beta $ but also $p_{0}$ that is not Lorentz Invariant. In fact, it's the energy of the particle \emph{j}, and with a boost it will be different.\\
We have shown that 
\[
